...
# dev/staging project (see DOCS/SSOT/09_DEPLOYMENT.md).
...
# For this repo, schema.sql is the canonical full local bootstrap. The migrations
# folder contains incremental changes only and does not recreate the base schema
# on a fresh disposable Postgres instance.
if [ ! -f "$SCHEMA_FILE" ]; then
  echo "Schema file $SCHEMA_FILE not found in repo — aborting." >&2
  exit 1
fi
echo "Applying schema snapshot: $SCHEMA_FILE to local DB (127.0.0.1:${PG_PORT})"
PGPASSWORD="${PG_PASS}" psql "postgresql://postgres:${PG_PASS}@127.0.0.1:${PG_PORT}/postgres?sslmode=disable" -v ON_ERROR_STOP=1 -f "$SCHEMA_FILE"

-- Restore search_trainers schema (featured flags + integer review_count)
-- Drop older overloads first so the CREATE can define a new return shape safely.
DROP FUNCTION IF EXISTS public.search_trainers(numeric, numeric, integer, age_specialty, behavior_issue[], service_type, boolean);
DROP FUNCTION IF EXISTS public.search_trainers(numeric, numeric, age_specialty[], behavior_issue[], service_type, boolean, boolean, text, numeric, text, integer, integer);

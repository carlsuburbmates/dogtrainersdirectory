...
- `GET /api/admin/scaffolded`: may surface the existing scaffold-review shadow guidance candidate and bounded next-safe-action checks directly on the admin decision surface, but that guidance remains advisory only and must not auto-approve, auto-reject, publish, verify, feature, spotlight, bill, or re-rank a listing.

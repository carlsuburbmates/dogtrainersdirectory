...
- `CS-1004` is now the active priority again. The publish path truth and live locality alignment are repaired, but the bounded live apply still fails for `trainer` rows because the current publish sequence inserts `businesses` before the required `trainer_specializations` linkage.
...
- Phase 17 - Concierge Seeding & Scraping is not yet complete for the current planned slice because the accepted publish path still needs the trainer-specialization sequencing correction before the approved concierge batch can be published cleanly.

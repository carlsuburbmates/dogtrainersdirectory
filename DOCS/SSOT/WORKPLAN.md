...
- `CS-1003` is now complete and the concierge pipeline performs inventory-aware duplicate assessment plus schema-aligned relational mapping for the 19-row pilot, with the current live mapping packet reporting `5 mapping_ready / 12 needs_review / 2 blocked`.
- `CS-1004` is now complete again. The bounded concierge publish path now resolves live locality truth at apply time and inserts trainer-linked rows transactionally so approved trainer candidates can publish under the live specialization contract.
- `CS-1005` is now complete and the existing launch verifier continues to enforce the frozen concierge launch-gate contract against the live searchable surface.
- Phase 17 - Concierge Seeding & Scraping is complete for the current planned slice. The remaining blocker is inventory depth and geography coverage, not concierge publish mechanics.
...
**AO-914: Re-align admin to exceptions-first and reduce dashboard noise**
- Purpose: reduce weekly operator scanning burden by making `/admin` prioritise actionable exception loops and direct links to the resolving queue or action, while keeping degraded and supervision states explicit.
- Definition of done:
  - `/admin` prioritises actionable exception loops over duplicated status panels or low-value dashboard noise.
  - Exception summaries link the operator directly to the queue or action that resolves the issue instead of forcing cross-surface scanning.
...
- DTD remains directory-first. Triage is a routing layer on top of the directory rather than a narrower replacement for it.
...
- Directory inclusion, resource-type validation, and trainer-launch counting are separate decisions and must not be conflated by the Listings Session.

**CS-1001: Define the canonical concierge seed contract and review artifact (completed 2026-03-30)**
- Purpose: replace the legacy phase2 placeholder path with one canonical, reviewable input and pre-publish artifact for real directory inventory.
- Definition of done:
...
- Purpose: automate enrichment from human-approved directory source URLs into a schema-aligned, publish-ready candidate payload.
- Definition of done:
  - Automation fetches only human-approved source URLs and extracts the listing fields needed for MVP launch inventory.
  - The normalization path resolves canonical `suburb_id`, strips unsafe or low-signal formatting noise, and maps listing content into the DTD taxonomy shape without inventing unsupported claims.
...
**CS-1003: Add duplicate checks and relational mapping for publish-ready trainer candidates (completed 2026-03-31)**
...
- 2026-03-31: `CS-1003` completed after the bounded inventory-aware duplicate correction pass. The concierge pipeline now compares candidates against both the current batch and the existing public directory inventory via the canonical Supabase `search_trainers` read path, keeps raw contact extraction separate from encryption-required write payloads, and emits truthful review plus mapping artifacts. The verified live pilot packet remains `5 ready / 12 needs_review / 2 blocked`, the live mapping packet is `5 mapping_ready / 12 needs_review / 2 blocked`, the inventory duplicate check was available with `4` existing inventory rows, and `CS-1004` is now the active priority.
- 2026-03-31: `CS-1004` completed after adding the bounded concierge scaffolded publish path plus the matching admin approval truth fix. The new publish command inserts only `mapping_ready` candidates, keeps encrypted contact handling on the canonical `encrypt_sensitive` RPC path, and writes live listings as `is_scaffolded=true`, `is_claimed=false`, `verification_status='pending'`, and `abn_verified=false`. The adjacent `/api/admin/scaffolded` approval route now preserves that same pending/unclaimed truth state instead of auto-marking approved scaffolded listings as claimed or verified. Focused verification stayed green (`vitest` for the admin route, Python publish-path tests, and `docs:guard`), and `CS-1005` is now the active priority.
- 2026-03-31: Main-control froze the `CS-1005` launch-gate contract using currently accepted Phase 17 inputs. The MVP launch verifier must now enforce the Inner Melbourne (`Inner City`) threshold of `30` real searchable trainer listings across all `3` Inner City councils, at least `11` distinct Inner City suburbs, and minimum live taxonomy coverage of `3` `age_specialty` values, `4` `service_type` values, and `5` `behavior_issue` values. The 19-row pilot queue remains pipeline-proof inventory only and is not sufficient launch evidence on its own.
- 2026-03-31: `CS-1005` completed by extending the existing `scripts/verify_launch.ts` path with a bounded live inventory gate backed by the canonical `public.search_trainers` surface plus schema enrichment for the returned `business_id`s. Focused verification (`vitest`, `type-check`, `eslint`) passed, and the live `--inventory-only` packet reported `4` searchable rows, `0` counted listings, and a truthful `FAIL` launch state because all currently live-searchable rows are placeholder/demo-only inventory (`DTD Verification Trainer PH205` plus the three `DTD Demo Trainer ...` rows). Phase 17 is complete for the current planned slice, but launch readiness remains blocked until real searchable inventory replaces the demo baseline and clears the frozen gate.
- 2026-04-03: A bounded operational publish attempt for the current `5` `mapping_ready` concierge rows was explicitly authorized, dry-run verified (`5` ready / `14` skipped / `0` failed), and then attempted against the live project. All `5` apply attempts failed at the `businesses.suburb_id` foreign-key boundary because the canonical artifact’s target `suburb_id` values (`8`, `9`, `11`, `12`) are not present in the current live `suburbs` table. The rerun live launch packet stayed unchanged at `4` searchable rows, `0` counted listings, and `FAIL`. The next blocker is therefore live locality alignment, not more operator approval or concierge review on the same `5` rows.
- 2026-04-03: The live locality blocker is now resolved. The concierge publisher no longer trusts artifact-local numeric `suburb_id` values and instead resolves the live `suburb_id` at apply time from the canonical `resolved_suburb` + `resolved_council` identity carried by the review artifact. The live target was also repaired to contain the full canonical Inner City suburb set. A rerun bounded publish then successfully inserted `Kira Canine Code`, and the live launch packet moved to `5` searchable rows with `1` counted real listing. However, the remaining `4` approved `trainer` rows still fail on `Trainer businesses must have at least one specialization.`, which means the bounded trainer publish sequence is still incomplete and `CS-1004` is reopened as the active priority until specialization sequencing is fixed.
- 2026-04-04: Main-control corrected the Listings Session policy for post-Phase-17 inventory operations. DTD is now explicitly documented as directory-first, with triage acting as a routing layer rather than a narrower inclusion rule. Operator-verified concierge batches must separate directory inclusion, resource-type validation, and trainer-launch counting. Vet, hospital, emergency, and urgent-care listings remain valid DTD inventory when their `resource_type` and geography are truthful, even if they do not count toward the current trainer-specific launch gate.
...
- 2026-04-04: Main-control accepted the bounded `CS-1004` correction for transactional trainer publish sequencing. The live locality blocker remained resolved, the remaining four approved trainer rows published successfully (`business_id` 20-23), and the fresh launch inventory gate stayed truthful at `5 counted / 2 councils / 4 suburbs / FAIL`. Phase 17 implementation is complete again; the remaining blocker is inventory depth and geography coverage, not publish mechanics.
